/**
 * WebSocket client for connecting to the Augury production server.
...
export class AuguryWebSocket {
  ...
  public onMessage: ((reading: AuguryReading) => void) | null = null;
      ...
      this.ws.onmessage = (event) => {
        try {
          ...
          if (msg.type === "reading" && msg.payload && this.onMessage) {
            this.onMessage(msg.payload as AuguryReading);
          }
        } catch {
...
// Fetch recent readings via HTTP (backfill)
const API_URL =
  process.env.NEXT_PUBLIC_AUGURY_API_URL ||
  "https://augury-production-6543.up.railway.app";

export async function fetchRecentReadings(limit = 5): Promise<AuguryReading[]> {
  try {
    const res = await fetch(`${API_URL}/readings?limit=${limit}`);
    if (!res.ok) return [];
    const data = (await res.json()) as { readings: AuguryReading[] };
